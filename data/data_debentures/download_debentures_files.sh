...
for DEB in ${DEBS[@]} ; do
    ...
    [ -e ${TSV_PU_FILE_NAME} ] && cat ${TSV_PU_FILE_NAME} | gtail -n +3 | ghead --lines=-4 | sed 's/\.//g' | sed 's/,/./g' | tr '[\t]' '[,]' | sed 's/,$//' > ${CSV_PU_FILE_NAME}
    ...
    [ -e ${TSV_NEG_FILE_NAME} ] && cat ${TSV_NEG_FILE_NAME} | gtail -n +3 | sed 's/\.//g' | sed 's/,/./g' | tr '[\t]' '[,]' > ${CSV_NEG_FILE_NAME}
done
